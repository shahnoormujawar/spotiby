...
const commonArgs = () => { const c = cookiesFile(); return ['--no-warnings', ...(c ? ['--cookies', c] : []), ...jsRuntimeArgs()]; };
...
function startSource(y, url) {
  return new Promise((resolve, reject) => {
    const fmt = DIRECT ? '251/bestaudio[ext=webm]/bestaudio[acodec=opus]/bestaudio' : 'bestaudio/best';
    const run = y.exec([url, '-f', fmt, '--no-playlist', '--no-part', '-o', '-', ...commonArgs()], { stdio: ['ignore', 'pipe', 'pipe'] });
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    try {
      ...
      if (DIRECT) {
        ...
        proc.stdout.resume();
        return { title: c.title, url: c.url, stream: proc.stdout, cleanup, mime: 'audio/webm', ext: 'webm' };
      }
